<script lang="ts">
  ...
  import { check, x, triangle } from "../svgdata.json";
  ...
  let buttons_checked = $derived(
    buttons.map((vec) => ({
      vec: vec,
      chk: boxes.findIndex((bx) => bx.objType !== 0 && compvec(bx.position, vec)) >= 0,
    }))
  );
  ...
  const getbtncolor = (chk: boolean) => (chk ? "#99ff66" : "#ff0000");
</script>
...
>
      ...
      {#each row as land_type, colIndex}
        ...
        {#if land_type >= 3 && land_type <= 6}
          <svg
            ...
            fill="#ffcc00"
          >
            <path d={triangle.path} transform={`rotate(${90 * (land_type - 3)} 480 -480)`} />
          </svg>
        {/if}
      {/each}
    {/each}
  ...
  {#each buttons_checked as { vec, chk }}
    <svg
      ...
      class="button"
      fill={getbtncolor(chk)}
    >
      <path d={chk ? check.path : x.path} stroke={getbtncolor(chk)} stroke-width="8%" />
    </svg>
  {/each}
</div>
